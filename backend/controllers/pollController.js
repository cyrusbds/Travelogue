...
exports.votePoll = async (req, res) => {
  try {
    ...
    if (!poll.multipleSelection && optionIndexes.length > 1)
      return res
        .status(400)
        .json({ message: "This poll only allows one choice" });
    ...
    for (const idx of optionIndexes) {
      if (idx < 0 || idx >= poll.options.length)
        return res
          .status(400)
          .json({ message: `Invalid option index: ${idx}` });
    }
    ...
    if (req.io) {
      req.io
        .to(`trip:${tripId}:polls`)
        .emit("poll:vote_cast", { poll: serialised });
    }
...
exports.updatePoll = async (req, res) => {
  try {
    ...
    if (poll.creatorId.toString() !== userId)
      return res
        .status(403)
        .json({ message: "Only the poll creator can edit it" });

    // Can't edit once someone has voted
    if (poll.voters.length > 0)
      return res
        .status(400)
        .json({ message: "Cannot edit a poll that has votes" });
    ...
    if (Array.isArray(options) && options.length >= 2)
      poll.options = options.map((label) => ({
        label: String(label).trim(),
        votes: [],
      }));
...
exports.closePoll = async (req, res) => {
  try {
    ...
    if (!isCreator && !isOwner)
      return res
        .status(403)
        .json({ message: "Not authorised to close this poll" });
...
exports.deletePoll = async (req, res) => {
  try {
    ...
    if (!isCreator && !isOwner)
      return res
        .status(403)
        .json({ message: "Not authorised to delete this poll" });
