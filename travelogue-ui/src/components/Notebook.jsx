    ...
    <PanelShell
      title={
        <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
          {Icon.itinerary} Trip Itinerary
        </span>
      }
      subtitle={
        hasItems
          ? `${items.length} activit${items.length === 1 ? "y" : "ies"} planned`
      ...
      {!isLoading && !hasItems && (
        <div className="nb-empty">
          ...
          <p className="nb-empty-desc">
            Start building your itinerary — add flights, hotels, meals, or
        ...
        <div>
          ...
          <button className="nb-add-activity-btn" onClick={openCreate}>
            <span
              style={{ display: "inline-flex", alignItems: "center", gap: 5 }}
            >
              {Icon.plus} Add another activity
            </span>
          </button>
        </div>
      )}
      ...
      <Modal
        ...
        subtitle={
          editItem
            ...
            : "Add an event, place, or plan to the itinerary."
        }
        footer={
          <>
            <button
...
function CalendarPanel({ toast, trip }) {
      ...
      <div className="nb-calendar-layout">
        <div>
          <div className="nb-cal-top">
            ...
            <button className="nb-cal-nav" onClick={nextMonth}>
              ›
            </button>
          </div>
          <div className="nb-cal-grid">
            {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => (
              <div key={d} className="nb-cal-dow">
                {d}
              </div>
            ))}
            {cells.map((n, i) => {
              if (!n)
                return <div key={`e${i}`} className="nb-cal-cell empty" />;
              const key = cellKey(n);
            ...
            <div>
              <div className="nb-trip-progress-card">
                ...
                {[
                  ...
                  {
                    label: "Explore",
                    val: items.filter((i) => i.category === "explore").length,
                  },
                  {
                    label: "Food",
                    val: items.filter((i) => i.category === "food").length,
                  },
                  {
                    label: "Stay",
                    val: items.filter((i) => i.category === "stay").length,
                  },
                  {
                    label: "Transport",
                    val: items.filter((i) => i.category === "transport").length,
                  },
                ].map((r) => (
  ...
  function DayView() {
    ...
    function nextDay() {
      if (currentIdx < allDays.length - 1)
        setSelectedDay(allDays[currentIdx + 1]);
    }
      ...
      <div>
        ...
        >
          ...
          <div style={{ flex: 1, textAlign: "center" }}>
            ...
            <div style={{ fontSize: "0.7rem", color: "var(--muted)" }}>
              ...
              {conflicts.size > 0 && (
                <span style={{ color: "#FF5F57", marginLeft: 8 }}>
        ...
        {dayItems.length === 0 ? (
          <div className="nb-empty" style={{ padding: 30 }}>
            ...
            <button
              className="nb-empty-cta"
              onClick={() => openCreate(displayDay)}
            >
              <span
                style={{ display: "inline-flex", alignItems: "center", gap: 5 }}
              >
          ...
          <div>
            ...
            >
              <span
                style={{ display: "inline-flex", alignItems: "center", gap: 5 }}
              >
                {Icon.plus} Add activity to this day
              </span>
            </button>
          </div>
        )}
...
function VotingPanel({ toast, trip }) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const { poll } = await apiCreatePoll(tripId, {
      ...
      toast(e.message || "Failed to create poll");
    }
  }
  ...
  async function handleVote() {
    ...
    try {
      ...
      toast(e.message || "Failed to cast vote");
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="nb-vote-grid">
        {polls.map((v) => (
          <div key={v._id} className="nb-vote-card">
            <div className="nb-vcf-header">
              <span className="nb-vcf-emoji" style={{ display: "flex" }}>
                {Icon.vote}
              </span>
              <div className="nb-vcf-q">{v.question}</div>
              <span
                className={`nb-vcf-status ${v.status === "active" ? "nb-vcf-open" : "nb-vcf-closed"}`}
              >
            ...
            {v.options.map((o, i) => {
              const pct = v.totalVotes
                ? Math.round((o.voteCount / v.totalVotes) * 100)
                : 0;
              return (
                <div key={i} className="nb-vote-opt-row">
                  ...
                  <div className="nb-vote-track">
                    <div
                      className={`nb-vote-fill ${FILL_CLASSES[i % 4]}`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
        ...
        {!isLoading && (
          <div
            className="nb-vote-card nb-vote-add"
            onClick={() => setCreateOpen(true)}
          >
            <div
              style={{
      ...
      >
        <div className="nb-form-group">
          <label className="nb-form-label">Question</label>
          <input
            className="nb-form-input"
            placeholder="e.g. Where to eat on Day 2?"
            value={form.q}
        ...
        <div className="nb-form-group">
          <label className="nb-form-label">Option 1</label>
          <input
            className="nb-form-input"
            placeholder="First option..."
            value={form.o1}
            onChange={(e) => setForm((f) => ({ ...f, o1: e.target.value }))}
          />
        </div>
        <div className="nb-form-group">
          <label className="nb-form-label">Option 2</label>
          <input
            className="nb-form-input"
            placeholder="Second option..."
            value={form.o2}
            onChange={(e) => setForm((f) => ({ ...f, o2: e.target.value }))}
          />
        </div>
        <div className="nb-form-group">
          <label className="nb-form-label">Option 3 (optional)</label>
          <input
            className="nb-form-input"
            placeholder="Third option..."
            value={form.o3}
            onChange={(e) => setForm((f) => ({ ...f, o3: e.target.value }))}
          />
        </div>
      </Modal>
      ...
      <Modal
        ...
        title={voteModalPoll?.question || ""}
        subtitle="Pick an option to cast your vote."
        footer={
          <>
            <button
              className="nb-modal-btn nb-modal-btn-ghost"
      ...
      >
        {voteModalPoll?.options.map((o, i) => (
          <div
            key={i}
            className={`nb-vote-choice ${picked === i ? "selected" : ""}`}
            onClick={() => setPicked(i)}
          >
            <div className="nb-vote-radio" />
            {o.label}
          </div>
        ))}
      </Modal>
...
function PackingPanel({ toast, trip }) {
  ...
  function handleAdd() {
    ...
    setItems((p) => [
      ...p,
      { ...newItem, _id: tempId, text: form.name, checked: false, done: false },
    ]);
    setForm({ name: "", who: "" });
    setAddOpen(false);
  ...
  return (
    ...
    >
      <div className="nb-packing-layout">
        <div>
          <div className="nb-pack-progress">
            <div className="nb-ppb-label">
              <span>
                {done} / {total} items
              </span>
              <span>{pct}%</span>
            </div>
            <div className="nb-ppb-track">
              <div className="nb-ppb-fill" style={{ width: `${pct}%` }} />
            </div>
          </div>
          ...
          {!isLoading && items.length === 0 && (
            <div className="nb-empty" style={{ padding: 30 }}>
              ...
              <p className="nb-empty-desc">
                Add items to your shared packing list — everyone in the group
                can check them off.
              </p>
              <button className="nb-empty-cta" onClick={() => setAddOpen(true)}>
                <span
                  style={{
                    display: "inline-flex",
                    alignItems: "center",
              ...
              <div key={cat}>
                ...
                {items.map((it, i) => {
                  ...
                  return (
                    <div
                      key={it._id || i}
                      className={`nb-pack-item ${isDone ? "done" : ""}`}
                      onClick={() => toggle(i)}
                    >
                      <div className="nb-pack-checkbox">
                        {isDone && (
        ...
        <div>
          <div className="nb-packing-stat">
            <div className="nb-psc-num">{done}</div>
            <div className="nb-psc-label">Items Packed</div>
          </div>
          <div className="nb-packing-stat">
            <div className="nb-psc-num">{total - done}</div>
            <div className="nb-psc-label">Items Left</div>
          </div>
        </div>
      </div>
      ...
      <Modal
        ...
        subtitle="Add to the shared checklist."
        footer={
          <>
            <button
              className="nb-modal-btn nb-modal-btn-ghost"
      ...
      >
        <div className="nb-form-group">
          <label className="nb-form-label">Item Name</label>
          <input
            className="nb-form-input"
        ...
        <div className="nb-form-group">
          <label className="nb-form-label">Assigned To</label>
          <input
            className="nb-form-input"
            placeholder="e.g. Everyone, or leave blank"
            value={form.who}
      ...
      <div className="nb-chat-wrap">
        <div className="nb-chat-header">
          <div
            style={{
              ...
              border: "1px solid rgba(200,98,58,0.25)",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              color: "var(--terracotta)",
        ...
        <div className="nb-chat-messages" ref={messagesRef}>
          ...
          {messages.map((msg) => (
            <div
              key={msg._id}
              className={`nb-msg-group ${isMine(msg) ? "mine" : "theirs"}`}
            >
              <div className="nb-msg-author-row">
                <div
                  className="nb-msg-mini-av"
                  style={{ background: getBubbleColor(msg.sender?.name) }}
                >
                ...
                <span className="nb-msg-author-name">
                  ...
                  {msg.sender?.isGuest && (
                    <em style={{ fontSize: "0.65rem", color: "var(--muted)" }}>
                      {" "}
                      (guest)
                    </em>
                  )}
              ...
              >
                ...
                {msg.optimistic && (
                  <span
                    style={{ opacity: 0.4, fontSize: "0.65rem", marginLeft: 6 }}
                  >
                    sending…
                  </span>
                )}
...
function SharePanel({ toast, trip, user }) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      toast(e.message || "Failed to generate link");
    } finally {
      setCreating(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      {!isLoading && !isOwner && (
        <div className="nb-empty">
          ...
          <div className="nb-empty-title">
            Only the trip owner can manage invite links
          </div>
        </div>
      )}
      ...
      {links.map((link) => (
        ...
        >
            ...
            <span
              style={{
                ...
                background: link.active
                  ? "rgba(40,200,64,0.12)"
                  : "rgba(255,95,87,0.12)",
                color: link.active ? "#28C840" : "#FF5F57",
                border: `1px solid ${link.active ? "rgba(40,200,64,0.2)" : "rgba(255,95,87,0.2)"}`,
              }}
            >
          ...
          >
            <span>
              Role:{" "}
              <strong style={{ color: "var(--text-dim)" }}>{link.role}</strong>
            </span>
            <span>
              Uses:{" "}
          ...
          {link.active && (
            <div style={{ display: "flex", gap: 8 }}>
              ...
              >
                ...
                >
                  {copied === link._id ? Icon.check : Icon.copy}{" "}
                  {copied === link._id ? "Copied!" : "Copy Link"}
                </span>
              </button>
              <button
                className="nb-action-btn nb-action-ghost"
      ...
      <Modal
        ...
        subtitle="Configure access settings for this invite."
        footer={
          <>
            <button
              className="nb-modal-btn nb-modal-btn-ghost"
    ...
    <div className="nb-root">
      ...
      <nav className="nb-mobile-tabs">
        {ALL_TABS.map((t) => (
          <button
            key={t.id}
            className={`nb-mobile-tab ${active === t.id ? "active" : ""}`}
          ...
          {NAV.map((section) => (
            <div key={section.section}>
              ...
              {section.items.map((item) => (
                <button
                  key={item.id}
                  className={`nb-sidebar-item ${active === item.id ? "active" : ""}`}
