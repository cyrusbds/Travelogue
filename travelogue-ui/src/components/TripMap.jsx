...
const MI = {
  ...
  route: (
    <svg
      ...
      strokeLinecap="round"
    >
      <path d="M3 12h3M3 6h3M3 18h3M9 6h6a3 3 0 0 1 0 6H9v6h6a3 3 0 0 0 0-6" />
    </svg>
  ),
  locate: (
    <svg
...
export default function TripMap({ trip, toast }) {
  ...
  useEffect(() => {
    ...
    map.on("click", async (e) => {
      ...
      const tempMarker = L.marker([lat, lng], { icon: makeTempIcon() }).addTo(
        map,
      );
  ...
  useEffect(() => {
    ...
    pins.forEach((pin) => {
      ...
      if (markersRef.current[pin._id]) {
        markersRef.current[pin._id].setIcon(
          makePinIcon(pin.category, isActive),
        );
      } else {
        const color = CAT_COLOR[pin.category] || CAT_COLOR.explore;
        const marker = L.marker([pin.lat, pin.lng], {
          icon: makePinIcon(pin.category, isActive),
        }).addTo(map);
  ...
  useEffect(() => {
    ...
    if (pin?.lat && pin?.lng)
      leafletRef.current.flyTo([pin.lat, pin.lng], 14, { duration: 1.2 });
  }, [activePin]);
  ...
  async function savePin(lat, lng) {
    if (!form.name.trim()) {
      toast("Please enter a place name");
      return;
    }
    if (!tripId) {
      toast("No trip loaded");
      return;
    }
    ...
    try {
      ...
      toast(
        lat
          ? `📍 ${form.name.trim()} pinned!`
          : `${form.name.trim()} saved (no coordinates)`,
      );
      setForm({ name: "", desc: "", day: "Day 1", category: "explore" });
      setGeoResult(null);
      setClickModal(false);
  ...
  async function handleDelete(pinId, e) {
    ...
    try {
      ...
      const newPins = updated.pins ?? updated;
      setPins(
        Array.isArray(newPins) ? newPins : pins.filter((p) => p._id !== pinId),
      );
        ...
        <div className="nb-panel-topbar">
          <div>
            ...
            >
              ...
              {pins.filter((p) => p.lat && p.lng).length > 1
                ? ` · route through ${pins.filter((p) => p.lat && p.lng).length} stops`
                : ""}
              <span
                style={{
                  fontSize: "0.65rem",
              ...
              style={
                showRoute
                  ? {
                      ...
                      borderColor: "rgba(200,98,58,0.4)",
                    }
                  : {}
              }
            >
            ...
            <div style={{ position: "relative", overflow: "hidden" }}>
              ...
              <div className="nb-map-controls" style={{ zIndex: 800 }}>
                ...
                <button
                  ...
                  onClick={() => {
                    const p = pins.find((x) => x.lat && x.lng);
                    if (p) leafletRef.current?.flyTo([p.lat, p.lng], 8);
                  }}
              ...
              {pins.filter((p) => p.lat && p.lng).length === 0 && (
                ...
                >
                  ...
                  >
                    or use the "Pin Location" button above
                  </div>
                </div>
              )}
            </div>
                  ...
                  <div
                    ...
                    style={
                      activePin === pin._id
                        ? {
                            borderColor:
                              CAT_COLOR[pin.category] || "var(--terracotta)",
                            background: `${CAT_COLOR[pin.category] || CAT_COLOR.explore}10`,
                          }
                        : {}
                    }
                  >
                    <div className="nb-mlc-header">
                      <div
                        className="nb-mlc-color"
                        style={{
                          background:
                            CAT_COLOR[pin.category] || CAT_COLOR.explore,
                        }}
                      />
                      <div className="nb-mlc-name">{pin.name}</div>
                      <div
                        className="nb-mlc-day"
                        style={{
                          display: "flex",
                          alignItems: "center",
                          gap: 6,
                    ...
                    <div className="nb-mlc-tags">
                      <span
                        className={`nb-tag tag-${pin.category || "explore"}`}
                        style={{ marginTop: 4 }}
                      >
          ...
          <div className="nb-modal-box">
            ...
            <div className="nb-modal-footer">
              ...
              <button
                className="nb-modal-btn nb-modal-btn-primary"
                onClick={() =>
                  savePin(geoResult?.lat ?? null, geoResult?.lng ?? null)
                }
                disabled={saving}
                style={{ display: "inline-flex", alignItems: "center", gap: 6 }}
              >
